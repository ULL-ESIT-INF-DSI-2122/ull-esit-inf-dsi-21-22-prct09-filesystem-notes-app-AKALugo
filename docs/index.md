...
## Ejercicio.
En este ejercicio tendremos que crear diferentes clases para realizar diferentes funciones ya que queremos respetar los principios SOLID, para empezar podemos de la clase nota.ts que es el corazón de este programa ya que es el encargado de almacenar la información de las diferentes notas, el código desarrollado es el siguiente:
...
Primero vamos a hablar del método `añadirNota`, como su propio nombre indica este método nos permitirá añadir una nota, para ello primero tendremos que ver si el usuario tiene un directorio donde almacenar sus notas y si no lo tiene se lo creamos, una vez hecho lo antiror solo nos quedaría mirar si la nota que quiere añadir ya existe para ese usuario, si es así no le permitimos que la añada pero si no es así la añadimos. Añadir una nota implica crear un fichero `.json` con el nombre de la nota en el directorio del usuario, para escribir los datos de la nota en formato .json utilizamos la clase que hemos creado para ello.
...
Para terminar de explicar el código que he desarrollado en esta práctica vamosa comentar el código que se encarga de leer los datos desde la consola y ejecutar los difentes métodos, este es el código desarrollado:
